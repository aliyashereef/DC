...
+ (NSDate *)dateFromSourceString:(NSString *)sourceString {
    ...
    NSArray *dateFormatterList = [NSArray arrayWithObjects:EMIS_DATE_FORMAT,
                                 @"yyyy-MM-dd HH:mm:ss", @"yyyy-MM-dd HH:mm:ss.SSS", @"yyyy-MM-dd'T'HH:mm:ss",@"dd MMM,yyyy HH:mm", @"d-MMM-yyyy HH:mm", TWENTYFOUR_HOUR_FORMAT, nil];//include all possible date formats here
    if (sourceString) {
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
